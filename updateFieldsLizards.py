...
INPUT_DIR = "./airdrop"
OUTPUT_DIR = "./updatedAirdrop"

for file_name in os.listdir(INPUT_DIR):
    if ".png" in file_name:
        # # renames png file to a number
        new_file_name = int(file_name.split(".")[0]) - 880
        os.rename(f"{INPUT_DIR}/{file_name}", f"{OUTPUT_DIR}/{new_file_name}.png")
    else:
        new_file_name = int(file_name.split(".")[0]) - 880
        os.rename(f"{INPUT_DIR}/{file_name}", f"{OUTPUT_DIR}/{new_file_name}.json")
